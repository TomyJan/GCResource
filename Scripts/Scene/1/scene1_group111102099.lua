...
-- 装置
gadgets = {
	{ config_id = 99001, gadget_id = 70330256, pos = { x = 1967.688, y = 215.504, z = -1596.851 }, rot = { x = 0.000, y = 3.666, z = 0.000 }, level = 32, state = GadgetState.GearStart, persistent = true },
	{ config_id = 99002, gadget_id = 70330283, pos = { x = 1985.467, y = 220.911, z = -1566.276 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 32, persistent = true, server_global_value_config = {SGV_CHAINTYPE = 100004, SGV_CHAINLEVEL = 5} }
}
...
suites = {
	{
		...
		gadgets = { 99001, 99002 },
		regions = { },
		triggers = { "SELECT_OPTION_99003", "VARIABLE_CHANGE_99004", "GROUP_LOAD_99005", "SELECT_OPTION_99006", "VARIABLE_CHANGE_99007", "GROUP_LOAD_99008" },
		rand_weight = 100
	}
}
...
function condition_EVENT_SELECT_OPTION_99003(context, evt)
	-- 判断是gadgetid 99002 option_id 439
...
function action_EVENT_SELECT_OPTION_99003(context, evt)
	-- 将configid为 99002 的物件更改为状态 GadgetState.GearAction2
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 99002, GadgetState.GearAction2) then
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_99005(context, evt)
	-- 判断变量"pressbutton1"为1
	if ScriptLib.GetGroupVariableValue(context, "pressbutton1") ~= 1 then
			return false
	end
...
-- 触发操作
function action_EVENT_GROUP_LOAD_99005(context, evt)
	-- 将configid为 99001 的物件更改为状态 GadgetState.Default
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 99001, GadgetState.Default) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
...
-- 触发条件
function condition_EVENT_SELECT_OPTION_99006(context, evt)
	-- 判断是gadgetid 99002 option_id 438
	if 99002 ~= evt.param1 then
		return false	
	end
...
-- 触发操作
function action_EVENT_SELECT_OPTION_99006(context, evt)
	-- 将configid为 99002 的物件更改为状态 GadgetState.GearAction2
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 99002, GadgetState.GearAction2) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
...
-- 触发条件
function condition_EVENT_VARIABLE_CHANGE_99007(context, evt)
	if evt.param1 == evt.param2 then return false end
...
-- 触发操作
function action_EVENT_VARIABLE_CHANGE_99007(context, evt)
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_99008(context, evt)
	-- 判断变量"pressbutton1"为0
	if ScriptLib.GetGroupVariableValue(context, "pressbutton1") ~= 0 then
			return false
	end
...
-- 触发操作
function action_EVENT_GROUP_LOAD_99008(context, evt)
	-- 将configid为 99001 的物件更改为状态 GadgetState.GearStart
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 99001, GadgetState.GearStart) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
