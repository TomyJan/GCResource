...
-- 怪物
monsters = {
	{ config_id = 9001, monster_id = 26010301, pos = { x = -2546.368, y = 223.166, z = -4155.860 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 27, drop_tag = "骗骗花", area_id = 11 }
}
...
-- 装置
gadgets = {
	{ config_id = 9002, gadget_id = 70510013, pos = { x = -2546.368, y = 223.166, z = -4155.860 }, rot = { x = 0.000, y = 319.800, z = 0.000 }, level = 32, persistent = true, area_id = 11 }
}
...
triggers = {
	...
	{ config_id = 1009005, name = "GADGET_CREATE_9005", event = EventType.EVENT_GADGET_CREATE, source = "", condition = "condition_EVENT_GADGET_CREATE_9005", action = "action_EVENT_GADGET_CREATE_9005", trigger_count = 0 },
...
suites = {
	{
		...
		gadgets = { 9002 },
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_9003", "ANY_MONSTER_DIE_9004", "GADGET_CREATE_9005", "SELECT_OPTION_9006" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
function action_EVENT_GADGET_STATE_CHANGE_9003(context, evt)
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 133220009, 2)
...
-- 触发条件
function condition_EVENT_GADGET_CREATE_9005(context, evt)
	if 9002 ~= evt.param1 or GadgetState.Default ~= ScriptLib.GetGadgetStateByConfigId(context, 0, evt.param1) then
		return false
	end
...
-- 触发操作
function action_EVENT_GADGET_CREATE_9005(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 133220009, 9002, {173}) then
...
function condition_EVENT_SELECT_OPTION_9006(context, evt)
	-- 判断是gadgetid 9002 option_id 173
...
function action_EVENT_SELECT_OPTION_9006(context, evt)
	...
	-- 删除指定group： 133220009 ；指定config：9002；物件身上指定option：173；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 133220009, 9002, 173) then
