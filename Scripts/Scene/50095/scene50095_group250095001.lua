...
-- 怪物
monsters = {
	{ config_id = 1005, monster_id = 24030101, pos = { x = -0.011, y = -0.102, z = 9.971 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, disableWander = true, pose_id = 101 },
	{ config_id = 1006, monster_id = 24030201, pos = { x = -0.071, y = -0.102, z = -10.039 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, pose_id = 101 }
}
...
function action_EVENT_SELECT_OPTION_1002(context, evt)
	...
	-- 删除指定group： 250095001 ；指定config：1001；物件身上指定option：175；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 250095001, 1001, 175) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 250095001, 2)
...
function action_EVENT_ANY_MONSTER_LIVE_1010(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 166, 180, 250095001, 2, 0) then
...
function action_EVENT_CHALLENGE_FAIL_1014(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 250095001, suite = 1 }) then
