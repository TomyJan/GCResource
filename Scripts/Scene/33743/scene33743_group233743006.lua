...
function action_EVENT_SELECT_OPTION_6004(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 233743007, {7001,7002,7003,7004,7005,7006,7007,7008,7009,7011,7021,7022}, 12, 1, 4) then
		return -1
	end
	
	-- 删除指定group： 233743006 ；指定config：6001；物件身上指定option：175；
...
function action_EVENT_SELECT_OPTION_6005(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 233743008, {8001,8002,8003,8004,8005,8006,8007,8008,8011}, 9, 3, 3) then
		return -1
	end
	
	-- 创建编号为2（该怪物潮的识别id)的怪物潮，创建怪物总数为8，场上怪物最少2只，最多2只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 233743008, {8014,8015,8016,8017}, 8, 2, 2) then
		return -1
	end
	
	-- 删除指定group： 233743006 ；指定config：6001；物件身上指定option：176；
