...
-- 怪物
monsters = {
	{ config_id = 1008, monster_id = 22040201, pos = { x = 0.216, y = 0.192, z = -2.572 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 }, pose_id = 101 },
	{ config_id = 1011, monster_id = 22040201, pos = { x = 3.630, y = 0.192, z = -1.158 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 }, pose_id = 101 },
	{ config_id = 1012, monster_id = 22040201, pos = { x = -4.607, y = 0.192, z = -2.466 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 }, pose_id = 101 },
	{ config_id = 1013, monster_id = 22040201, pos = { x = -7.881, y = 0.192, z = -0.721 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 }, pose_id = 101 },
	{ config_id = 1016, monster_id = 22050201, pos = { x = -2.164, y = 0.192, z = -2.059 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 }, pose_id = 101 },
	{ config_id = 1019, monster_id = 21011001, pos = { x = 9.311, y = 0.192, z = -1.308 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 } },
	{ config_id = 1020, monster_id = 21011001, pos = { x = -9.797, y = 0.192, z = -2.634 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 } }
}
...
function action_EVENT_ENTER_REGION_1002(context, evt)
	-- 改变指定group组240808001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240808001, 1001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240808001, 1001, {7}) then
...
function action_EVENT_SELECT_OPTION_1003(context, evt)
	...
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 1001, GadgetState.GearStop) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240808002, suite = 2 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240808001, 2)
	
	-- 删除指定group： 240808001 ；指定config：1001；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240808001, 1001, 7) then
...
function action_EVENT_CHALLENGE_FAIL_1009(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240808002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	...
	-- 删除suite2的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 240808001, 2)
	
	-- 删除suite3的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 240808001, 3)
...
function action_EVENT_CHALLENGE_SUCCESS_1010(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240808002, suite = 0 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function action_EVENT_ANY_MONSTER_LIVE_1023(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 191, 360, 240808001, 7, 0) then
...
function condition_EVENT_ANY_MONSTER_DIE_1024(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 240808001) ~= 2 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_1024(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240808001, 3)
