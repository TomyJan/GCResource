...
-- 怪物
monsters = {
	{ config_id = 1008, monster_id = 20011101, pos = { x = 3.787, y = 0.192, z = -2.572 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1011, monster_id = 20011101, pos = { x = -3.503, y = 0.192, z = -2.476 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1012, monster_id = 21020701, pos = { x = -2.134, y = 0.192, z = -2.623 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1013, monster_id = 21010701, pos = { x = 4.348, y = 0.192, z = -5.548 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1014, monster_id = 21010701, pos = { x = 0.038, y = 0.192, z = -7.079 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1015, monster_id = 21010701, pos = { x = -4.090, y = 0.192, z = -5.155 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1016, monster_id = 20011001, pos = { x = 6.055, y = 0.192, z = 7.227 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1 },
	{ config_id = 1017, monster_id = 20011001, pos = { x = -0.042, y = 0.192, z = 8.881 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1 },
	{ config_id = 1018, monster_id = 20011001, pos = { x = -5.858, y = 0.192, z = 7.662 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1 },
	...
	{ config_id = 1022, monster_id = 21020701, pos = { x = 2.109, y = 0.192, z = -2.623 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
suites = {
	...
	},
	{
		-- suite_id = 2,
		-- description = ,
		monsters = { 1008, 1011, 1013, 1014, 1015 },
		gadgets = { },
		regions = { },
...
function action_EVENT_ENTER_REGION_1002(context, evt)
	-- 改变指定group组240807001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240807001, 1001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240807001, 1001, {7}) then
...
function action_EVENT_SELECT_OPTION_1003(context, evt)
	...
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 1001, GadgetState.GearStop) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240807002, suite = 2 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240807001, 2)
	
	-- 删除指定group： 240807001 ；指定config：1001；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240807001, 1001, 7) then
...
function action_EVENT_CHALLENGE_FAIL_1009(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240807002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	...
	-- 删除suite2的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 240807001, 2)
	
	-- 删除suite3的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 240807001, 3)
	
	-- 删除suite4的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 240807001, 4)
...
function action_EVENT_CHALLENGE_SUCCESS_1010(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240807002, suite = 0 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function action_EVENT_ANY_MONSTER_LIVE_1023(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 191, 360, 240807001, 13, 0) then
...
function condition_EVENT_ANY_MONSTER_DIE_1024(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 240807001) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_1024(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240807001, 3)
...
function condition_EVENT_ANY_MONSTER_DIE_1025(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 240807001) ~= 2 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_1025(context, evt)
	-- 添加suite4的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240807001, 4)
