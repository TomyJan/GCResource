...
-- 装置
gadgets = {
	{ config_id = 13001, gadget_id = 70360002, pos = { x = 18.396, y = -4.102, z = -41.684 }, rot = { x = 0.000, y = 299.272, z = 0.000 }, level = 1, is_guest_can_operate = true },
...
function action_EVENT_GADGET_CREATE_13004(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 245002013, 13001, {4007}) then
...
function action_EVENT_SELECT_OPTION_13005(context, evt)
	-- 删除指定group： 245002013 ；指定config：13001；物件身上指定option：4007；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 245002013, 13001, 4007) then
	...
	-- 延迟10秒后,向groupId为：245002013的对象,请求一次调用,并将string参数："close" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 245002013, "close", 10) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_timerevent_by_group")
	  return -1
	end
	
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 245002013, EntityType.GADGET, 13002 ) then
