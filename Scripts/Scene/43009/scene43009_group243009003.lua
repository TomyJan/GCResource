...
-- 装置
gadgets = {
	{ config_id = 3001, gadget_id = 70360164, pos = { x = 32.233, y = 40.189, z = -45.690 }, rot = { x = 0.000, y = 31.000, z = 0.000 }, level = 1, mark_flag = 4 },
	{ config_id = 3002, gadget_id = 70350244, pos = { x = 32.560, y = 39.906, z = -52.873 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, state = GadgetState.GearStart },
	{ config_id = 3003, gadget_id = 70360161, pos = { x = 71.359, y = 40.000, z = -29.049 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 3004, gadget_id = 70360160, pos = { x = -1.550, y = 40.000, z = -75.364 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 3005, gadget_id = 70360162, pos = { x = 17.949, y = 40.000, z = -3.029 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 3006, gadget_id = 70360159, pos = { x = 32.233, y = 69.733, z = -45.690 }, rot = { x = 0.000, y = 188.508, z = 0.000 }, level = 1, mark_flag = 2 },
	{ config_id = 3007, gadget_id = 70900201, pos = { x = 31.981, y = 41.664, z = -45.607 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 3008, gadget_id = 70900201, pos = { x = 32.733, y = -19.540, z = -51.837 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}

-- 区域
regions = {
	{ config_id = 3012, shape = RegionShape.SPHERE, radius = 5, pos = { x = 18.416, y = 39.998, z = -4.530 } },
	-- 切换天气区域
	{ config_id = 3015, shape = RegionShape.CUBIC, size = { x = 40.000, y = 1.000, z = 40.000 }, pos = { x = 31.734, y = -19.539, z = -51.370 } },
	-- 全局启动trigger
	{ config_id = 3018, shape = RegionShape.CUBIC, size = { x = 40.000, y = 20.000, z = 2.000 }, pos = { x = 16.352, y = 40.000, z = 13.368 } },
	-- 断线重连保护大region
	{ config_id = 3025, shape = RegionShape.CUBIC, size = { x = 300.000, y = 200.000, z = 300.000 }, pos = { x = 39.568, y = -13.000, z = -18.604 } }
}
...
function action_EVENT_DUNGEON_ALL_AVATAR_DIE_3009(context, evt)
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009001, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009005, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009006, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009007, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009008, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009011, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	...
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009012, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
...
function action_EVENT_TIME_AXIS_PASS_3010(context, evt)
	-- 重新生成指定group，默认为独立随机
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243009009 }) then
...
function action_EVENT_TIMER_EVENT_3014(context, evt)
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 243009003, EntityType.GADGET, 3002 ) then
...
function action_EVENT_SELECT_OPTION_3017(context, evt)
	...
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 243009003, "floor", 2) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_timerevent_by_group")
	  return -1
	end
	
	-- 删除中控台group的地上操作台物件；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 243009003, 3001, 59) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
...
function action_EVENT_ENTER_REGION_3018(context, evt)
	...
	ScriptLib.StartGallery(context, 7037)
