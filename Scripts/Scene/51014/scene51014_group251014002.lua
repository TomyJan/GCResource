...
function action_EVENT_CHALLENGE_SUCCESS_2001(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 251014003, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_2002(context, evt)
	-- 改变指定group组251014001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 251014001, 1001, GadgetState.Default) then
	...
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 251014001, 1001, {7}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 251014002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function action_EVENT_ANY_MONSTER_LIVE_2009(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 191, 300, 251014002, 12, 0) then
