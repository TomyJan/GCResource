...
-- 怪物
monsters = {
	{ config_id = 7001, monster_id = 20070101, pos = { x = -0.032, y = -0.102, z = -0.013 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, affix = { 9009 } }
}
...
function action_EVENT_ANY_MONSTER_LIVE_7011(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 197, 600, 235735007, 1, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_7014(context, evt)
	-- 将剩余时间记录在触发此challenge的group变量TPL_TIME中
	    if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "TPL_TIME", evt.param2, 235735005) then
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 235735005) then
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 235735006, 6001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 235735006, 6001, {176}) then
...
function action_EVENT_CHALLENGE_FAIL_7015(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 235735007, suite = 1 }) then
