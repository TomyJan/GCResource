...
-- 怪物
monsters = {
	{ config_id = 1008, monster_id = 25210202, pos = { x = 3.787, y = 0.192, z = -2.572 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 } },
	{ config_id = 1011, monster_id = 25310301, pos = { x = -0.030, y = 0.192, z = -0.263 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 } },
	{ config_id = 1012, monster_id = 25210202, pos = { x = -4.607, y = 0.192, z = -2.466 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 } },
	{ config_id = 1013, monster_id = 25210301, pos = { x = 4.348, y = 0.192, z = 1.830 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 } },
	{ config_id = 1015, monster_id = 25210403, pos = { x = -4.090, y = 0.192, z = 2.223 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 } },
	{ config_id = 1016, monster_id = 25310201, pos = { x = 2.996, y = 0.192, z = 0.229 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 }, pose_id = 1 },
	{ config_id = 1017, monster_id = 25310101, pos = { x = -3.101, y = 0.192, z = 0.214 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 1007 }, pose_id = 1 }
}
...
-- 装置
gadgets = {
	{ config_id = 1001, gadget_id = 70360010, pos = { x = 0.111, y = 0.036, z = 3.128 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, state = GadgetState.GearStop }
}
...
function action_EVENT_ENTER_REGION_1002(context, evt)
	-- 改变指定group组240652001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240652001, 1001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240652001, 1001, {7}) then
...
function action_EVENT_SELECT_OPTION_1003(context, evt)
	...
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 1001, GadgetState.GearStop) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240652002, suite = 2 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240652001, 2)
	
	-- 删除指定group： 240652001 ；指定config：1001；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240652001, 1001, 7) then
...
function action_EVENT_CHALLENGE_FAIL_1009(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240652002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	...
	-- 删除suite2的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 240652001, 2)
	
	-- 删除suite3的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 240652001, 3)
...
function action_EVENT_ANY_MONSTER_LIVE_1023(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 191, 360, 240652001, 7, 0) then
...
function condition_EVENT_ANY_MONSTER_DIE_1024(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 240652001) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_1024(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240652001, 3)
