...
function action_EVENT_GADGET_CREATE_1022(context, evt)
	...
	-- 重新生成指定group，默认为独立随机
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 247204008 }) then
...
function action_EVENT_VARIABLE_CHANGE_1026(context, evt)
	...
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 247204001, 3)
...
function action_EVENT_SELECT_OPTION_1028(context, evt)
	...
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 247204001, 2)
	
	
	-- 删除指引点
	    ScriptLib.RemoveExtraGroupSuite(context, 247204001, 3)
	
	-- 删除中控台group的地上操作台物件；物件身上指定option：59；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 247204001, 1003, 59) then
...
function condition_EVENT_VARIABLE_CHANGE_1032(context, evt)
	...
	-- 判断变量"success"为1
	if ScriptLib.GetGroupVariableValueByGroup(context, "success", 247204001) ~= 1 then
			return false
	end
