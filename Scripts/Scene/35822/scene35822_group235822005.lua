...
-- 装置
gadgets = {
	{ config_id = 5001, gadget_id = 70360010, pos = { x = -59.273, y = 13.203, z = 85.709 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_GADGET_CREATE_5002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 235822005, 5001, {177}) then
...
function condition_EVENT_SELECT_OPTION_5003(context, evt)
	...
	-- 判断变量"stage"为0
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 235822004) ~= 0 then
			return false
	end
...
function action_EVENT_SELECT_OPTION_5003(context, evt)
	-- 创建编号为1（该怪物潮的识别id)的怪物潮，创建怪物总数为18，场上怪物最少3只，最多3只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 235822006, {6001,6002,6003,6004,6005,6006,6007,6008,6009,6010,6011,6012,6013,6014,6024,6025,6026,6027}, 18, 3, 3) then
		return -1
	end
	
	-- 删除指定group： 235822005 ；指定config：5001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 235822005, 5001, 177) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 改变指定group组235822005中， configid为5001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 235822005, 5001, GadgetState.GearStop) then
