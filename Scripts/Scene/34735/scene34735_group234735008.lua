...
-- 怪物
monsters = {
	{ config_id = 8001, monster_id = 23010101, pos = { x = 4.018, y = -0.102, z = -11.035 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true },
	...
	{ config_id = 8004, monster_id = 23030102, pos = { x = -2.929, y = -0.102, z = 9.957 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, disableWander = true },
	{ config_id = 8005, monster_id = 23020102, pos = { x = 0.046, y = -0.102, z = 9.943 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, disableWander = true, affix = { 6105 }, isElite = true },
	{ config_id = 8017, monster_id = 23040102, pos = { x = 3.090, y = -0.102, z = 9.943 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, disableWander = true }
}
...
function action_EVENT_ANY_MONSTER_LIVE_8010(context, evt)
	...
	-- 从234735005的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 234735005)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 234735008, 6, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_8012(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 234735005) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_groupVariable_by_group")
	  return -1
	end
	
	-- 改变指定group组234735009中， configid为9001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 234735009, 9001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组234735009中， configid为9002的gadget的state
...
function action_EVENT_CHALLENGE_FAIL_8013(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 234735008, suite = 1 }) then
...
function action_EVENT_ANY_MONSTER_DIE_8020(context, evt)
	...
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 234735008, 3)
