...
function action_EVENT_ANY_MONSTER_DIE_3010(context, evt)
	-- 添加suite4的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241052003, 4)
...
function action_EVENT_ANY_MONSTER_DIE_3012(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241052003, 3)
...
function action_EVENT_ANY_MONSTER_DIE_3013(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241052003, {3017,3015,3016,3018,3019}, 5, 5, 5) then
		return -1
	end
...
function action_EVENT_ANY_MONSTER_LIVE_3022(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 1, 241052003, 17, 0, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_3023(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 241052001) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_groupVariable_by_group")
	  return -1
	end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241052002, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_3024(context, evt)
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241052003, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
