...
-- 怪物
monsters = {
	{ config_id = 2001, monster_id = 24030201, pos = { x = 0.009, y = 9.325, z = -9.027 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, affix = { 1048 }, pose_id = 101 },
	{ config_id = 2007, monster_id = 24030101, pos = { x = 0.009, y = 9.128, z = -11.740 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, affix = { 1048 }, pose_id = 101 }
}
...
-- 装置
gadgets = {
	{ config_id = 2021, gadget_id = 70900205, pos = { x = 5.692, y = 7.442, z = 5.984 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_2022(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 191, 300, 240752002, 2, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_2023(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240752003, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_2024(context, evt)
	...
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240752001, 1001, {7}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240752002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function condition_EVENT_ANY_MONSTER_DIE_2025(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 240752002) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_2025(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240752002, 3)
