...
-- 怪物
monsters = {
	{ config_id = 8001, monster_id = 20040701, pos = { x = 0.051, y = -0.102, z = -0.020 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true }
}
...
function action_EVENT_ANY_MONSTER_LIVE_8010(context, evt)
	...
	-- 从235743005的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 235743005)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 235743008, 1, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_8012(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 235743005) then
...
function action_EVENT_CHALLENGE_FAIL_8013(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 235743008, suite = 1 }) then
