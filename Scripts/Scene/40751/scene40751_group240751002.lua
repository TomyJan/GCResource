...
-- 怪物
monsters = {
	{ config_id = 2001, monster_id = 24020201, pos = { x = -3.934, y = 9.213, z = -7.323 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, pose_id = 100 },
	{ config_id = 2002, monster_id = 24020401, pos = { x = 3.620, y = 9.213, z = -7.408 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, pose_id = 100 },
	{ config_id = 2007, monster_id = 24030201, pos = { x = 0.065, y = 9.213, z = -11.202 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, pose_id = 101 }
}
...
-- 装置
gadgets = {
	{ config_id = 2021, gadget_id = 70900205, pos = { x = 5.748, y = 7.440, z = 5.925 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_2022(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 191, 300, 240751002, 3, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_2023(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240751003, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_2024(context, evt)
	-- 改变指定group组240751001中， configid为1001的gadget的state
	...
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240751001, 1001, {7}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240751002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function condition_EVENT_ANY_MONSTER_DIE_2025(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 240751002) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_2025(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240751002, 3)
